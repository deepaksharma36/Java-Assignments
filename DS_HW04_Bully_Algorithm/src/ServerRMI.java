...
public class ServerRMI {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int holderID = Integer.parseInt(args[2]);
			Process exportedProcess = new Process(processID,holderID,RMIPortNum);
			...
			exportedProcess.electionMessageResponder();
			exportedProcess.coordinatorMessageMonitor();
			exportedProcess.electionMonitor();
			
			System.out.println("Press Enter when all Process are running");
			Scanner sc = new Scanner(System.in);
			sc.nextLine();
			exportedProcess.leaderHeartBeatMonitor();
			exportedProcess.stateFliper();
